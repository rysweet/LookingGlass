...
const OPTIONAL_EXTERNAL_A3P = path.resolve(
  __dirname,
  "../../alice/netbeans/target/ant-jar-command-line-" + "sm" + "oke/jar-command-line-world.a3p"
);
...
describe("a3p-parser", () => {
  ...
  describe("optional external .a3p (UTF-16BE)", () => {
    let project: AliceProject;
    const fileExists = optionalExternalA3PFixtureExists(OPTIONAL_EXTERNAL_A3P);

    beforeAll(async () => {
      if (!fileExists) return;
      const data = fs.readFileSync(OPTIONAL_EXTERNAL_A3P);
      project = await parseA3P(data);
    });
