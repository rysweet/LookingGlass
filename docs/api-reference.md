...
Providing `modelResourceId` makes the object use the
imported model asset stored in project state. Unknown model resource IDs return
`400`.

## `POST /api/assets/import-model`

Import a `.gltf` or `.glb` model into the current project. This route
uses JSON base64 uploads with a 25 MiB body limit; the general API JSON limit is
not large enough for model uploads.
...
Import a browser-compatible image texture into the current project. This route uses the same 25 MiB JSON body limit as model imports.
