...
| POST | `/api/screenshot` | `{}` | success: `{ status: "captured", path, objectCount, sceneDescription, rendered: true }`; fallback: `{ status: "captured", path, placeholder: true, error }` | renderer exceptions fall back to placeholder PNG | `renderSceneToPng()` |
