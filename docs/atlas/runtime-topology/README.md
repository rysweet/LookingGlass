...
- `src/cli.ts` is the planned process entry for the local server CLI. A checkout runs it through `npm run serve --`; the installed binary is `alice-web serve`. It binds the API to `127.0.0.1:3000` by default and passes `evidenceDir`, `project`, and `apiToken` options into `createServer()`.
